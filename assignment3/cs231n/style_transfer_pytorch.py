...
def gram_matrix(features, normalize=True):
    ...
    gram_mat = torch.einsum('bcij,bdij->bcd', features, features)
    return gram_mat / features[0].numel() if normalize else gram_mat
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, layer_idx in enumerate(style_layers):
        gram_mat_c = gram_matrix(feats[layer_idx])
        loss += style_weights[idx] * torch.sum(torch.square(gram_mat_c - style_targets[idx]))
    return loss
...
def tv_loss(img, tv_weight):
    ...
    return tv_weight * (torch.sum(torch.square(pixel_diff1)) + torch.sum(torch.square(pixel_diff2)))
