...
def tv_loss(img, tv_weight):
    ...
    return tv_weight * (tf.reduce_sum(tf.square(pixel_dif1)) + tf.reduce_sum(tf.square(pixel_dif2)))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, layer_idx in enumerate(style_layers):
        gram_mat_c_feats = gram_matrix(feats[layer_idx])
        loss += style_weights[idx] * tf.reduce_sum(tf.square(style_targets[idx] - gram_mat_c_feats))
    return loss
...
def gram_matrix(features, normalize=True):
    ...
    gram_mat = tf.linalg.einsum('bijc,bijd->bcd', features, features)
    return gram_mat / (H*W*C) if normalize else gram_mat
