...
def compute_saliency_maps(X, y, model):
    ...
    ds_X = tape.gradient(correct_scores, X)
    saliency = tf.math.reduce_max(tf.math.abs(ds_X), axis=-1)
...
def make_fooling_image(X, target_y, model):
    ...
    for it in range(100):
        with tf.GradientTape() as tape:
            ...
            correct_scores = tf.gather(scores, target_y, axis=-1)
        print('Predicted clas: {} VS. target cls: {}'
            .format(tf.math.argmax(scores, axis=-1), target_y))
        if tf.math.argmax(scores, axis=-1) == target_y:
            break
        dX = tape.gradient(correct_scores, X_fooling)
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
        tape.watch(X)
        scores = model(X)
        score_y = tf.gather(scores, target_y, axis=-1)
        obj = score_y - l2_reg * tf.math.square(tf.norm(X))
    dX = learning_rate * tape.gradient(obj, X)
    X += dX
