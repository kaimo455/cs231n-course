...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad = conv_param.get('pad', (max(w.shape[-2:]) - 1) // 2)
    x_pad = np.pad(x, ((0,), (0,), (pad,), (pad,)))  # [N, C_in, H+2*pad, W+2*pad]
    x_pad = x_pad[:, np.newaxis, ...]  # [N, 1, C_in, H+2*pad, W+2*pad]
    N, C_in, H_in, W_in = x.shape
    F, _, H_k, W_k = w.shape
    H_out = 1 + (H_in + 2*pad - H_k) // stride
    W_out = 1 + (W_in + 2*pad - W_k) // stride
    ...
    for _h in range(H_out):
        ...
        for _w in range(W_out):
            w_begin = _w * stride
            w_end = w_begin + W_k
            _x_pad = x_pad[..., h_begin:h_end, w_begin:w_end]  # [N, 1, C_in, H_k, W_k]
            out[:, :, _h, _w] = np.sum(_x_pad * w, axis=(-3, -2, -1))
...
def conv_backward_naive(dout, cache):
    ...
    _, _, H_k, W_k = w.shape
    x_pad = np.pad(x, ((0,), (0,), (pad,), (pad,)))  # [N, C_in, H+2*pad, W+2*pad]
    # ------- backward propagation - db ------- start
    db = np.sum(dout, axis=(0, -2, -1))
    # ------- backward propagation - db ------- end
    dw = np.zeros_like(w)  # [F, C_in, H_k, W_k]
    dx_pad = np.zeros_like(x_pad)  # [N, C_in, H+2*pad, W+2*pad]
    for _h in range(H_out):
        h_begin = _h * stride
        h_end = h_begin + H_k
        for _w in range(W_out):
            w_begin = _w * stride
            w_end = w_begin + W_k
            # ------- backward propagation - dw ------- start
            d_x_pad_w = dout[:, :, _h, _w][..., np.newaxis, np.newaxis, np.newaxis]  # [N, F, 1, 1, 1]
            _x_pad = x_pad[:, np.newaxis, :, h_begin:h_end, w_begin:w_end]  # [N, 1, C_in, H_k, W_k]
            dw += np.sum(d_x_pad_w * _x_pad, axis=0) # [N, F, 1, 1, 1] * [N, 1, C_in, H_k, W_k] -> [N, F, C_in, H_k, W_k]
            # ------- backward propagation - dw ------- end
            # ------- backward propagation - dx_pad ------- start
            d_x_pad = d_x_pad_w * w[np.newaxis, ...]  # [N, F, 1, 1, 1] * [1, F, C_in, H_k, W_k] -> [N, F, C_in, H_k, W_k]
            dx_pad[..., h_begin:h_end, w_begin:w_end] += np.sum(d_x_pad, axis=1)
            # ------- backward propagation - dx_pad ------- end
    # ------- backward propagation - dx ------- start
    dx = dx_pad[:, :, pad:-pad, pad:-pad]
    # ------- backward propagation - dx ------- end
...
def max_pool_forward_naive(x, pool_param):
    ...
    for _h in range(H_out):
        ...
        for _w in range(W_out):
            ...
            out[:, :, _h, _w] = np.max(x[:, :, h_begin:h_end, w_begin:w_end], axis=(-2, -1))
...
def max_pool_backward_naive(dout, cache):
    ...
    for _h in range(H_out):
        ...
        for _w in range(W_out):
            w_begin = _w * stride
            w_end = w_begin + pool_width
            x_patch = x[:, :, h_begin:h_end, w_begin:w_end]
            _idx = (x_patch == np.max(x_patch, axis=(-2, -1), keepdims=True))
            dx[:, :, h_begin:h_end, w_begin:w_end][_idx] += dout[:, :, _h, _w].ravel(order='C')
...
def spatial_batchnorm_backward(dout, cache):
    ...
    _dout = _dout.reshape(-1, dout.shape[1])
    dx, dgamma, dbeta = batchnorm_backward_alt(_dout, cache)
    dx = dx.reshape(dout.shape[0], dout.shape[2], dout.shape[3], dout.shape[1])
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    Inputs:
    - x: Input data of shape (N, C, H, W)
    - gamma: Scale parameter, of shape (C,) / (1, C, 1, 1)
    - beta: Shift parameter, of shape (C,) / (1, C, 1, 1)
